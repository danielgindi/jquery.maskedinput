...
(function (root, factory) {
    ...
    function MaskedInput() {
        if (!(this instanceof MaskedInput)) {
            ...
            return new (Function.prototype.bind.apply(MaskedInput, [MaskedInput].concat(Array.prototype.slice.call(arguments, 0))))();
        }
    ...
    MaskedInput.prototype.initialize = function (options) {
        var that = this,
            o = that.o = $.extend({}, MaskedInput.defaults, options),
            patterns = {};
        ...
        // Set control data
        $el.data('control', that).data('maskedinput', that);
    ...
    var FORMAT_REGEX = new RegExp('(0+(?::[a-zA-Z0-9_]+)?)' + /* numeric value, fixed length, with possible :name_123 */
        ...
        $.each(p.parsed, function () {
            ...
            if (part.name && parseInt(part.name, 10).toString() != part.name) {
                if (inputs.hasOwnProperty(part.name)) {
                    inputs[part.name] = inputs[part.name] instanceof HTMLElement ? [inputs[part.name], part.el] : inputs[part.name].concat([part.el]);
                } else {
    ...
    MaskedInput.prototype._syncInputSize = function (input, alwaysConsiderPlaceholder, fallbackText) {
        ...
        $backBuffer.css($input.css(inputBackbufferCssProps)).text(value).appendTo(that.$el);

        // Measure these
        var backBufferWidth = getPreciseContentWidth($backBuffer[0]) + 1 /* caret width */,
            currentWidth = getPreciseContentWidth(inputEl);
        ...
        if (alwaysConsiderPlaceholder && inputEl.value && inputEl.placeholder && inputEl.placeholder != inputEl.value) {
            ...
            backBufferWidth = Math.max(backBufferWidth, getPreciseContentWidth($backBuffer[0]) + 1 /* caret width */
            );
        }
    ...
    MaskedInput.prototype._handleKeydown = function (event, input, part) {
        ...
        if (keycode === KEY_ARROW_UP || keycode === KEY_ARROW_DOWN) {
            ...
            var nextValue,
                tryToUpdate = false,
                minLen = part.maxLength ? Math.max(1, Math.min(part.length || 0, part.maxLength || 1)) : part.length || 1,
                maxLen = Math.max(part.length || 0, part.maxLength || 0);

            if (part.type === PartType.TEXT && part.options) {

                var fullMatch = findMatchInArray(part.options, contentBefore, true, true, false),
                    index = part.options.indexOf(fullMatch);
    ...
    MaskedInput.prototype._handleKeypress = function (event, input, part) {
        ...
        var keycode = event.which,
            pos = getSelectionRange(input);


        if (event.ctrlKey || event.altKey || event.metaKey || !keycode || keycode < 32 || keycode === KEY_ENTER) return that; // Not a character, perform default

        event.preventDefault();

        var triggerChange = false,
            moveToNextField = false,
            pressedChar = event.key || String.fromCharCode(keycode),
            contentBefore = input.value,
            contentAfter = contentBefore.substr(0, pos.begin) + pressedChar + contentBefore.substr(pos.end),
            validatedContent = that._validateContent(contentAfter, part);
